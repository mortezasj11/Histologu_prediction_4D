...
class WeightedFocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets, weight=None):
        BCE_loss = nn.functional.binary_cross_entropy_with_logits(inputs, targets, pos_weight = weight , reduction=self.reduction)
...
hparams = { 'lr' : 5e-3,
            ...
            'max_epochs' : 300,
            'train_batch' : 3,
            'weight' : torch.tensor([923/(316+923), 316/(316+923) ]),#torch.tensor([1.0, 1.0 ])
            'reduction':'none',#"none", 'mean'
            'schedular_step_size':1, 
            'schedular_gamma':0.9,
            'lambda_l1':2e-8,
            'in_channels': 3,
            'Aug': 'simple',
            'name':'_focal_V1_weight_sum',
            'GPU_number': '0'}
...
def main():
    ...
    train_os = pd.read_csv('/Data/Histology/Nature_Gemini_4d_Feb2022/TrainValTest/NatureGemini2/train.csv')
    val_os   = pd.read_csv('/Data/Histology/Nature_Gemini_4d_Feb2022/TrainValTest/NatureGemini2/val.csv')
    test_os  =  pd.read_csv('/Data/Histology/Nature_Gemini_4d_Feb2022/TrainValTest/NatureGemini2/test.csv')
    ...
    if hparams["Aug"].lower()=='simple':
        ...
        print('{} Aug is being used.'.format(hparams["Aug"]))
        rotate_range, shear_range, translate_range =((-np.pi, np.pi), (-np.pi, np.pi), (-np.pi, np.pi)), (-5, 5, -5, 5,-5, 5), (0.05, 0.05, 0.05)
    train_transforms = Compose(
            [
                ...
                RandAdjustContrastd(keys=["input"], prob=0.1),
                RandZoomd(keys="input", prob=rand_p, min_zoom=0.85, max_zoom=1.15),
                EnsureTyped(keys=["input"]),
                RandAffined(keys=["input"], prob=rand_p, rotate_range=rotate_range, shear_range=shear_range, translate_range=translate_range)
            ]
        )
    ...
    val_loader = DataLoader(val_ds, batch_size=val_batch, shuffle=True, num_workers=20, collate_fn=list_data_collate, pin_memory=torch.cuda.is_available())

    # load the test data
    test_ds = Dataset(data=test_files, transform=val_transforms)
    test_loader = DataLoader(test_ds, batch_size=test_batch, shuffle=True, num_workers=20, collate_fn=list_data_collate,
    ...
    #loss_CE = torch.nn.CrossEntropyLoss(weight=hparams['weight'].cuda(), reduction='sum')
    loss_CE = WeightedFocalLoss(reduction=hparams["reduction"])

    optimizer = torch.optim.Adam(model.parameters(), lr)
    #optimizer = torch.optim.LBFGS(params, lr=1, max_iter=20, max_eval=None, tolerance_grad=1e-07, tolerance_change=1e-09, history_size=100, line_search_fn=None)
    ...
    if model_train:
        ...
        writer = SummaryWriter(log_dir='runs_NG_focal/' + time_stamp + hparams['name'])
        ...
        for epoch in t:
            ...
            for batch_data in train_loader:
                ...
                weight = torch.tensor([hparams['weight'][0] if x==0 else hparams['weight'][1] for x in t_label ]).unsqueeze(dim=1).cuda()
                t_loss_label = loss_CE(output4, t_label.float(),weight=weight )
                t_loss = t_loss_label + hparams['lambda_l1'] * torch.log10(l1_reg)

                t_loss_label_vec = t_loss_label.unsqueeze(0) if step == 0 else torch.cat((t_loss_label_vec, t_loss_label.unsqueeze(0)),dim=0)
                t_loss_vec = t_loss.unsqueeze(0) if step == 0 else torch.cat((t_loss_vec, t_loss.unsqueeze(0)),dim=0)
                #del inputs #del t_histology
                ###################################################################################################
                if verbose:
                ...
                #writer.add_scalar("Z_L1 loss: step", l1_reg.item(),                epoch_len * epoch + step)
                #writer.add_scalar("Train/label loss: step", t_loss_label.item(), epoch_len * epoch + step)
            writer.add_scalar("Train/overall_loss", t_loss_vec.mean().item(),      epoch)
            writer.add_scalar("Train/LabelLoss", t_loss_label_vec.mean().item(),  epoch )

            with torch.no_grad():
                epoch_loss /= step
                epoch_loss_values.append(epoch_loss)
                _, t_label_pred_for_acc = torch.max(train_labelpred,1)
                t_label_pred = train_labelpred
                t_acc = accuracy_score(t_label_pred_for_acc.cpu().numpy(), train_label.cpu().numpy())

                
                cm = confusion_matrix(train_label.cpu().detach().numpy(), t_label_pred_for_acc.cpu().detach().numpy() )
                figure = plot_confusion_matrix(cm, class_names=['Sq','Ad_Other'])
                _, torch_im = plt2arr(figure)

                figure2 = plot_confusion_matrix(cm, class_names=['Sq','Ad_Other'],normalize=False)
                _, torch_im2 = plt2arr(figure2)
                t_two_cm = torch.cat((torch_im[:,0:3,:,:],torch_im2[:,0:3,:,:]), dim =3)
                plt.close()
                writer.add_scalar("Train/Accuracy",       t_acc.item(),                   epoch)
                writer.add_images('Train/CM',                    t_two_cm,                       epoch)
                ...
                #writer.add_scalar("hparam/loss_weight_2",        hparams['weight'][2],           epoch)
                ...
                ## plotting ROC
                #train_label.shape : torch.Size([492, 1])
                #train_labelpred.shape : torch.Size([492, 2])

                ''' y_t = np.array([ [1,0] if l==0 else [0,1] for l in train_label.cpu().numpy() ])
                y_p = train_labelpred.cpu().numpy()
                ...
                tpr = dict()
                roc_auc = dict()
                fpr["micro"], tpr["micro"], _ = roc_curve(train_label.cpu().numpy(),train_labelpred.cpu().numpy())
                roc_auc["micro"] = auc(fpr["micro"], tpr["micro"])

                '''
                figure3 = plot_ROC(fpr[0], tpr[0],roc_auc[0])
                _, torch_im2 = plt2arr(figure3)
                writer.add_images('Train/ROC0',                    torch_im2[:,0:3,:,:],                       epoch)
                figure3 = plot_ROC(fpr[1], tpr[1],roc_auc[1])
                _, torch_im2 = plt2arr(figure3)
                writer.add_images('Train/ROC1',                    torch_im2[:,0:3,:,:],                       epoch)
                '''
                figure3 = plot_ROC(fpr["micro"], tpr["micro"],roc_auc["micro"])
                _, torch_im2 = plt2arr(figure3)
                writer.add_images('Train/ROC_micro',               torch_im2[:,0:3,:,:],                  epoch)
              #################################    VALIDATION    ###################################
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_data in val_loader:
                        ...
                        weight_v = torch.tensor([hparams['weight'][0] if x==0 else hparams['weight'][1] for x in v_label ]).unsqueeze(dim=1).cuda()
                        v_loss_label = loss_CE(val_output4, v_label.float(), weight=weight_v)
                    ...
                    _, v_label_pred_for_acc = torch.max(val_labelpred,1)
                    v_label_pred = val_labelpred
                    v_acc = accuracy_score(v_label_pred_for_acc.cpu().numpy(), val_label.cpu().numpy())
                    
                    cm_v = confusion_matrix(val_label.cpu().detach().numpy(), v_label_pred_for_acc.cpu().detach().numpy() )
                    ...
                    fpr = dict()
                    tpr = dict()
                    fpr["micro"], tpr["micro"], _ = roc_curve(val_label.cpu().numpy(), val_labelpred.cpu().numpy())
                    roc_auc["micro"] = auc(fpr["micro"], tpr["micro"])
                    '''
                    figure3 = plot_ROC(fpr[0], tpr[0],roc_auc[0])
                    _, torch_im2 = plt2arr(figure3)
            ...
            if model_test:
                ...
                with torch.no_grad():
                    for test_data in test_loader:
                        ...
                        weight_s = torch.tensor([hparams['weight'][0] if x==0 else hparams['weight'][1] for x in s_label ]).unsqueeze(dim=1).cuda()
                        s_loss_label = loss_CE(s_output4, s_label.float(), weight=weight_s)
                    ...
                    _, s_label_pred_for_acc = torch.max(test_labelpred,1)
                    s_label_pred = test_labelpred
                    s_acc = accuracy_score(s_label_pred_for_acc.cpu().numpy(), test_label.cpu().numpy())
                    writer.add_scalar("Val_2/Accuracy", s_acc.item(), epoch)
                    cm = confusion_matrix(test_label.cpu().detach().numpy(), s_label_pred_for_acc.cpu().detach().numpy() )
                    ...
                    tpr = dict()
                    roc_auc = dict()
                    fpr["micro"], tpr["micro"], _ = roc_curve(test_label.cpu().numpy(), test_labelpred.cpu().numpy())
                    roc_auc["micro"] = auc(fpr["micro"], tpr["micro"])
                    '''
                    figure3 = plot_ROC(fpr[0], tpr[0],roc_auc[0])
                    _, torch_im2 = plt2arr(figure3)
