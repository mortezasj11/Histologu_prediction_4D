...
def model_eval_gpu(model,datafile,val_transforms):
    ...
    with torch.no_grad():
        ...
        for batch_data in data_loader:
            ...
            OSevents = batch_data["OS"].cuda()
            OSdurations = batch_data["OStime"].cuda()
            PFSevents = batch_data["PFS"].cuda()
            PFSdurations = batch_data["PFStime"].cuda()
...
def main():
    ...
    BEST_MODEL_NAME = "multitask_10_29_model1-rep1"
    ...
    if CLAHE_label:
        train_dir = '/Data/Clahe/Wildtype_Cohort/Train/Input'
        val_dir = '/Data/Clahe/Wildtype_Cohort/Valid/Input'
        test_dir = '/Data/Clahe/Wildtype_Cohort/Test/Input'
        test2_dir =  '/Data/Clahe/Independent_Cohort'
    else:
        train_dir = '/Data/Non_clahe/Wildtype/Train/Input'
        val_dir = '/Data/Non_clahe/Wildtype/Valid/Input'
        test_dir = '/Data/Non_clahe/Wildtype/Test/Input'
    ...
    test2_img = sorted(glob.glob(os.path.join(test2_dir, "Patient_*.nii.gz")))

    train_os = pd.read_csv('/Data/train_event_2nd_ed.csv')
    val_os = pd.read_csv('/Data/valid_event_2nd_ed.csv')
    test_os = pd.read_csv('/Data/test_event_2nd_ed.csv')
    # test2_os = pd.read_csv('/Data/test_event_2nd_ed.csv')

    train_os.sort_values('ID')
    val_os.sort_values('ID')
    test_os.sort_values('ID')

    # get_target = lambda df: (df['OS_event'].values, df['OS'].values)
    # y_train_surv = get_target(train_os)
    # y_val_surv = get_target(val_os)
    # y_test_surv = get_target(test_os)

    train_files = [{"input": in_img, "OS": df1, "OStime": df2, "ID": df3, "PFS": df4, "PFStime": df5,
                    "LMet": df6, "AMet": df7, "BoMet": df8, "BrMet": df9, "LNMet": df10, "Age": df11, "Gender": df12,
                    "ADC": df13, "Smoker": df14, "stage": df15}
                   for in_img, df1, df2, df3, df4, df5, df6, df7, df8, df9, df10, df11, df12, df13,
                       df14, df15 in
                   zip(train_img, train_os["OS"], train_os["OStime"], train_os["ID"], train_os["PFS"],
                       train_os["PFStime"],
    ...
    val_files = [{"input": in_img, "OS": df1, "OStime": df2, "ID": df3, "PFS": df4, "PFStime": df5,
                    "LMet": df6, "AMet": df7, "BoMet": df8, "BrMet": df9, "LNMet": df10, "Age": df11, "Gender": df12,
                    "ADC": df13, "Smoker": df14, "stage": df15}
                   for in_img, df1, df2, df3, df4, df5, df6, df7, df8, df9, df10, df11, df12, df13,
                       df14, df15 in
                   zip(val_img, val_os["OS"], val_os["OStime"], val_os["ID"], val_os["PFS"],
                       val_os["PFStime"],
    ...
    test_files = [{"input": in_img, "OS": df1, "OStime": df2, "ID": df3, "PFS": df4, "PFStime": df5,
                    "LMet": df6, "AMet": df7, "BoMet": df8, "BrMet": df9, "LNMet": df10, "Age": df11, "Gender": df12,
                    "ADC": df13, "Smoker": df14, "stage": df15}
                   for in_img, df1, df2, df3, df4, df5, df6, df7, df8, df9, df10, df11, df12, df13,
                       df14, df15 in
                   zip(test_img, test_os["OS"], test_os["OStime"], test_os["ID"], test_os["PFS"],
                       test_os["PFStime"],
    ...
    if model_train:
        ...
        for epoch in t:
            ...
            for batch_data in train_loader:
                inputs = batch_data["input"].cuda()
                t_OS,t_OStime,t_PFS,t_PFStime = batch_data["OS"].cuda(),batch_data["OStime"].cuda(),\
                                                batch_data["PFS"].cuda(),batch_data["PFStime"].cuda()
                t_Age,t_Gender,t_ADC,t_Smoker = batch_data["Age"].cuda(),batch_data["Gender"].cuda(),\
                                                batch_data["ADC"].cuda(),batch_data["Smoker"].cuda()
                t_LMet, t_AMet, t_BoMet, t_BrMet, t_LNMet = batch_data["LMet"].cuda(), batch_data["AMet"].cuda(), \
                                                            batch_data["BoMet"].cuda(), batch_data["BrMet"].cuda(), \
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_data in val_loader:
                        ...
                        v_inputs, v_OS, v_OStime = val_data["input"].cuda(), val_data["OS"].cuda(), val_data["OStime"].cuda()
                        v_PFS, v_PFStime = val_data["PFS"].cuda(), val_data["PFStime"].cuda()
                        v_Age, v_Gender, v_ADC, v_Smoker = val_data["Age"].cuda(), val_data["Gender"].cuda(), \
                                                           val_data["ADC"].cuda(), val_data["Smoker"].cuda()
                        v_LMet, v_AMet, v_BoMet, v_BrMet, v_LNMet = val_data["LMet"].cuda(), val_data["AMet"].cuda(), \
                                                                    val_data["BoMet"].cuda(), val_data["BrMet"].cuda(), \
                                                                    val_data["LNMet"].cuda()
                    ...
                    for test_data in test_loader:
                        ...
                        s_inputs, s_OS, s_OStime = test_data["input"].cuda(), test_data["OS"].cuda(), test_data["OStime"].cuda()
                        s_PFS, s_PFStime = test_data["PFS"].cuda(), test_data["PFStime"].cuda()
                        s_Age, s_Gender, s_ADC, s_Smoker = test_data["Age"].cuda(), test_data["Gender"].cuda(), \
                                                           test_data["ADC"].cuda(), test_data["Smoker"].cuda()
                        s_LMet, s_AMet, s_BoMet, s_BrMet, s_LNMet = test_data["LMet"].cuda(), test_data["AMet"].cuda(), \
                                                                    test_data["BoMet"].cuda(), test_data["BrMet"].cuda(), \
