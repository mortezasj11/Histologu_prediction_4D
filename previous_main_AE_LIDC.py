...
def model_eval_gpu(model,datafile,val_transforms):
    ...
    with torch.no_grad():
        ...
        for batch_data in data_loader:
            ...
            t_label = batch_data["stage"].cuda()
            _,_,_,output4,output5,feature  = model(inputs)
...
def main():
    ...
    train_dir = '/Data/LIDC/5_TrainVal/train1'
    val_dir = '/Data/LIDC/5_TrainVal/val1'
    test_dir = '/Data/LIDC/5_TrainVal/val1'

    train_img = sorted(glob.glob(os.path.join(train_dir, "LIDC-*.nii.gz")))
    val_img = sorted(glob.glob(os.path.join(val_dir, "LIDC-*.nii.gz")))
    test_img = sorted(glob.glob(os.path.join(test_dir, "LIDC-*.nii.gz")))

    train_os = pd.read_csv('/Data/LIDC/5_TrainVal/train1/train.csv')
    val_os   = pd.read_csv('/Data/LIDC/5_TrainVal/val1/val.csv')
    test_os  = pd.read_csv('/Data/LIDC/5_TrainVal/val1/val.csv')
    ...
    train_files = [{"input": in_img, "stage": df15}
                   for in_img, df15 in
                   zip(train_img, train_os["Malignancy"])]

    val_files = [{"input": in_img,  "stage": df15}
                 for in_img, df15 in
                 zip(val_img,  val_os["Malignancy"])]

    test_files = [{"input": in_img,  "stage": df15}
                  for in_img, df15 in
                  zip(test_img, test_os["Malignancy"])]
    ...
    model = Fn_model_LIDC.AutoEncoder_New(
        dimensions=3,
        in_channels=1,
        out_channels=1,
        num_res_units=2,
    ...
    if model_train:
        ...
        writer = SummaryWriter(log_dir='runs/' + time_stamp + 'NoL1_Using_precision_posWeight_p6')
        ...
        for epoch in t:
            ...
            for batch_data in train_loader:
                ...
                if step == 0:
                    ...
                    train_label = torch.cat((train_label, t_label),0)
                    train_labelpred = torch.cat((train_labelpred, output4),0)
                #print('train_labelpred.shape',train_labelpred.shape,train_labelpred)
                #print('train_label.shape',train_label.shape, train_label)
                ...
                '''
                ### Me added
                t_loss = loss_func( t_loss_label, t_loss_img) + 0 * torch.log10(l1_reg)/1e8
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_data in val_loader:
                        ...
                        '''ME
                        v_OS, v_OStime = val_data["OS"].cuda(), val_data["OStime"].cuda()
                    ...
                    cm = confusion_matrix(val_label.cpu().detach().numpy(), v_label_pred.cpu().detach().numpy() )
                    figure = plot_confusion_matrix(cm, class_names=['0','1'])
                    _, torch_im = plt2arr(figure)
                    writer.add_images('Val/CM', torch_im[:,0:3,:,:], epoch)
                    ###
                    ...
                    if model_test:
                        for test_data in test_loader:
                            ...
                            '''ME
                            s_OS, s_OStime =  test_data["OS"].cuda(), test_data["OStime"].cuda()
