...
class Dense_model(nn.Module):
    def __init__(self,drop_rate, in_channels):
        ...
        self.drop_layer2 = nn.Dropout(p=drop_rate)
        self.fc3 = torch.nn.Linear(16, 2)

    def forward(self, x):  # define network
        encoded = self.backbone(x)
        encoded = self.drop_layer0(encoded)
        label128 = self.fc1(encoded)
        label128 = self.drop_layer1(label128)
        label16 = self.fc2(label128)
        #label16 = self.drop_layer2(label16)
        label = self.fc3(label16)
...
def main():
    ...
    if hparams["Aug"].lower()=='simple':
        ...
        print('{} Aug is being used.'.format(hparams["Aug"]))
        rotate_range, shear_range, translate_range =((-np.pi/6, np.pi/6), (-np.pi/6, np.pi/6), (-np.pi/6, np.pi/6)), (-0.05, 0.05, -0.05, 0.05,-0.05, 0.05), (0.04, 0.04, 0.04)
