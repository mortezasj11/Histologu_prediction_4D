...
class AutoEncoder_New(nn.Module):
    def __init__(
        ...
        self.fc4 = torch.nn.Linear(1024, 3) #ME, before: self.fc4 = torch.nn.Linear(1024, 9)
    ...
    def forward(self, x: torch.Tensor) -> Any:
        x = self.encode(x) #[B, 256, 8, 8, 8]
        encoded = self.intermediate(x) #([B, 256, 8, 8, 8]
        x = self.decode(encoded) #[B, 1, 128, 128, 128])
        x1 = self.survivalnet(encoded)  # [B, 1024] Gives Error for batch size of 1???????
        label = self.fc4(x1)# [B, 1024]
        f = self.bn(encoded)
        return label, x, f