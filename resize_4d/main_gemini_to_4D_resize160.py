...
lung_path = '/Data/Histology/Nature_Gemini_4d_Apr2022/Gemini_clipped_ctTumorLung_Johof'#-v /rsrch1/ip_rsrch/wulab/Wu:/DataS
save_path = '/Data/Histology/Nature_Gemini_4d_Apr2022/Gemini_4d_4_15_2022'    #-v /rsrch1/ip_rsrch/wulab/Mori:/Data
...
if __name__=='__main__':
    ct_list = [i for i in os.listdir(lung_path) if i.endswith('_0000_seg.nii.gz')]
    bias = 4000
    for i,ct_name in enumerate(ct_list):
        
        print('{}/{}.  {}'.format(i+1, len(ct_list),ct_name))
        whole_path_ct        = os.path.join(lung_path, ct_name)
        whole_path_seg       = os.path.join(lung_path, ct_name[:-16]+'_lung.nii.gz')
        whole_path_seg_Tumor = os.path.join(lung_path, ct_name[:-16]+'_tumor.nii.gz')
        ...
        # Saving
        save_path_name = os.path.join(save_path,ct_name[:-16] + '.nii.gz')
