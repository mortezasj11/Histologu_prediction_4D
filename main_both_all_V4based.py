...
from monai.transforms import (AsChannelFirstd,Compose,LoadImaged,Lambdad,RandAffined,RandAdjustContrastd,RandZoomd,EnsureTyped)
...
def main(hparams):
    ...
    train_os = pd.read_csv('/Data/Histology/Nature_Gemini_4d_Feb2022/TrainValTest/NatureGeminiAll_ageN/train.csv')   #train_os.sort_values('ID')
    val_os   = pd.read_csv('/Data/Histology/Nature_Gemini_4d_Feb2022/TrainValTest/NatureGeminiAll_ageN/val.csv')    #val_os.sort_values('ID')
    test_os  =  pd.read_csv('/Data/Histology/Nature_Gemini_4d_Feb2022/TrainValTest/NatureGeminiAll_ageN/val.csv')    #test_os.sort_values('ID')


    #train_os = pd.read_csv('/Data/Histology/Nature_Gemini_4d_Feb2022/TrainValTest/NatureGeminiAll_NoTobacco/train.csv')   #train_os.sort_values('ID')
    #val_os   = pd.read_csv('/Data/Histology/Nature_Gemini_4d_Feb2022/TrainValTest/NatureGeminiAll_NoTobacco/val.csv')    #val_os.sort_values('ID')
    #test_os  =  pd.read_csv('/Data/Histology/Nature_Gemini_4d_Feb2022/TrainValTest/NatureGeminiAll_NoTobacco/test.csv')    #test_os.sort_values('ID')
    ...
    train_files = [{"input": in_img, "histology": hist, "age":age, "smoking":smoking, "gender": gender}
                for in_img, hist, age, smoking, gender in
                zip(list_path_t, list_hist_t, list_age_t, list_smoking_t, list_gender_t)]
    ...
    val_files = [{"input": in_img, "histology": hist, "age":age, "smoking":smoking, "gender": gender}
                for in_img, hist, age, smoking, gender in 
                zip(list_path_v, list_hist_v, list_age_v, list_smoking_v, list_gender_v)]
    ...
    test_files = [{"input": in_img, "histology": hist, "age":age, "smoking":smoking, "gender": gender}
                for in_img, hist, age, smoking, gender in
                zip(list_path_s, list_hist_s, list_age_s, list_smoking_s, list_gender_s)]
    ...
    train_transforms = Compose(
            ...
            RandZoomd(keys="input", prob=rand_p, min_zoom=0.85, max_zoom=1.25),
        ...
        for epoch in t:
            ...
            for train_data in train_loader:
                ...
                if hparams['l1_loss_mode']=='mul':
                    loss = loss_func(loss_hist, loss_age, loss_smoking, loss_gender)*torch.log10(l1_reg)
                elif hparams['l1_loss_mode']=='add':
                    loss = loss_func(loss_hist, loss_age, loss_smoking, loss_gender) + hparams['lambda_l1']*l1_reg
            ...
            if epoch%hparams["tensorbord_val_interval"]==0:
                ...
                t_two_hist_cm = t_hist_tensorboard.confusion_matrix(classes_name = ["SQ", "AD/other"])
            ...
            if epoch % hparams["tensorbord_val_interval"]==0:
                ...
                with torch.no_grad():
                    ...
                    for val_data in val_loader:
                        ...
                        # loss wrapper
                        if hparams['l1_loss_mode']=='mul':
                            loss_val = loss_func(loss_hist_val, loss_age_val, loss_smoking_val, loss_gender_val)*torch.log10(l1_reg)

                        elif hparams['l1_loss_mode']=='add':
                            loss_val = loss_func(loss_hist_val, loss_age_val, loss_smoking_val, loss_gender_val) + hparams['lambda_l1']*l1_reg
                    ...
                    t_two_hist_cm_val = t_hist_tensorboard_val.confusion_matrix(classes_name = ["SQ", "AD/other"])
            ...
            if model_test and epoch % hparams["tensorbord_val_interval"]==0:
                ...
                with torch.no_grad():
                    ...
                    for test_data in test_loader:
                        ...
                        # loss wrapper
                        if hparams['l1_loss_mode']=='mul':
                            loss_test = loss_func(loss_hist_test, loss_age_test, loss_smoking_test, loss_gender_test)*torch.log10(l1_reg)
                        elif hparams['l1_loss_mode']=='add':
                            loss_test = loss_func(loss_hist_test, loss_age_test, loss_smoking_test, loss_gender_test) + hparams['lambda_l1']*l1_reg
                    ...
                    t_two_hist_cm_test = t_hist_tensorboard_test.confusion_matrix(classes_name = ["SQ", "AD/other"])
...
val_batch = 2
docker run -it --rm --gpus all --shm-size=250G '''
