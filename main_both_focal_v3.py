...
class Dense_model(nn.Module):
    def __init__(self,drop_rate, in_channels):
        ...
        self.drop_layer0 = nn.Dropout(p=drop_rate)
    ...
    def forward(self, x):  # define network
        ...
        encoded = self.drop_layer0(encoded)
        label32 = self.fc1(encoded)
        label32 = self.drop_layer1(label32)
        label = self.fc2(label32) #torch.Size([3, 2])
        #label = torch.softmax(label,dim=1) # along with dimension 1, the sum is 1 : tensor([[0.5062, 0.4938],...
        return label
...
def focal_loss_from_bce_loss(bce_loss, alpha=1, gamma=2):
    pt = torch.exp(-bce_loss)
...
class FocalLoss(nn.Module):
    ...
    def forward(self, input_tensor, target_tensor):
        ...
        out = nn.functional.nll_loss(  (  (1-prob)**self.gamma  ) * log_prob, 
            target_tensor, 
            weight=self.weight,
...
hparams = { 'lr' : 1e-3,
            ...
            'schedular_gamma':0.9,
            'lambda_l1':3e-9, #not -7
            'in_channels': 3,
            'Aug': 'no_simple',
            'name':'_V3_GoalIsALittleOverfit_',
            'GPU_number': '2'}
...
def main():
    ...
    if hparams["Aug"].lower()=='simple':
        ...
        print('{} Aug is being used.'.format(hparams["Aug"]))
        rotate_range, shear_range, translate_range =((-np.pi/8, np.pi/8), (-np.pi/8, np.pi/8), (-np.pi/8, np.pi/8)), (-0.03, 0.03, -0.03, 0.03,-0.03, 0.03), (0.02, 0.02, 0.02)
    ...
    train_transforms = Compose(
            [
                ...
                RandAdjustContrastd(keys=["input"], prob=0.3, gamma=(0.9, 1.1)),
    ...
    loss_CE = FocalLoss(weight=hparams["weight"].cuda(), reduction=hparams["reduction"])
    ...
    if model_train:
        ...
        writer = SummaryWriter(log_dir='runs_NG_focal/' + time_stamp +"__"+hparams['name'] + hparams["Aug"] + "__" + str(hparams["rand_p"]*100) + "__" + str(hparams["GPU_number"]))
        ...
        for epoch in t:
            ...
            for batch_data in train_loader:
                ...
                t_loss = t_loss_label + hparams['lambda_l1'] * l1_reg
            ...
            with torch.no_grad():
                ...
                t_acc_b = balanced_accuracy_score(train_label.cpu().numpy(), t_label_pred.cpu().numpy() ) #sklearn.metrics.balanced_accuracy_score(y_true, y_pred
                ...
                ## plotting ROC
                #train_label.shape : torch.Size([1239, 1])
                #train_labelpred.shape : torch.Size([1239, 2])
            ...
            if epoch > skip_epoch_model:
                ...
                if metric1 > best_metric_pfs:
                    ...
                    torch.save(model.state_dict(), BEST_MODEL_NAME + hparams["name"] +"_PFS.pth")
